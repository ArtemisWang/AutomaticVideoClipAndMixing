...
def train(use_gpu, model_path, lr, epochs, model_prefix, batch_size):
    ...
    for e in range(0, epochs):
        ...
        for i, data_ in tqdm(enumerate(dataloader), desc="Training"):
            ...
            p, q, res_p, res_q = mm(100, p_input, q_input)
            out = t.cat([p, q], 2)
            target = t.cat([res_p, res_q], 2)
            ...
            loss.backward()
            if (i + 1) % 600 == 0:
                tqdm.write("----- Epoch %d Batch %d -- Loss %.4f" % (e + 1, i + 1, loss))
        ...
        #         if 0.84 < dev_acc <= 0.85 and learning_rate != lr * 0.1:
        #             learning_rate = lr * 0.1
        #             optimizer = t.optim.Adam(mm.parameters(), lr=learning_rate)
        #         elif dev_acc <= 0.84 and learning_rate != lr:
        #             learning_rate = lr
        #             optimizer = t.optim.Adam(mm.parameters(), lr=learning_rate)
        #         elif loss > 0.85 and learning_rate != lr * 0.01:
        #             learning_rate = lr * 0.01
        #             optimizer = t.optim.Adam(mm.parameters(), lr=learning_rate)
        #         if test_acc > max_test and test_acc > 0.85:
        #             t.save(mm.state_dict(), '%s.pth' % (model_prefix + '_maximum'))
...
if __name__ == '__main__':
    train(True, None, 0.01, 10, 'model/mm-2', 50)
