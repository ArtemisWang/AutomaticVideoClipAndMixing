...
import model_pytorch_0 as mp
import numpy as np
import pickle
from tqdm import tqdm
import build_dataset_pytorch_1 as build_dataset_pytorch
...
def train(use_gpu, model_path, lr, epochs, model_prefix, batch_size):
    ...
    for e in range(0, epochs):
        ...
        train_acc_list.append(t.Tensor(train_acc_i).mean(0))
    ...
    with open('result/accall_n.txt', 'w') as f:
        f.write(str(t.Tensor(train_acc_list).numpy())+'\t'+str(t.Tensor(dev_acc_list).numpy())+'\t'+str(t.Tensor(test_acc_list).numpy()))
...
if __name__ == '__main__':
    train(True, None, 0.0001, 10, 'model/mm-112', 50)
