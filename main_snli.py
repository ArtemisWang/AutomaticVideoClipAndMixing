...
import model_snli as mp
import numpy as np
import pickle
from tqdm import tqdm
import build_dataset_pytorch
from torch.utils.data import DataLoader

def train(use_gpu, model_path, lr, epochs, model_prefix, batch_size):
    dataset = build_dataset_pytorch.QuoraDataset_train('sick_data/train_data_pad.pkl','sick_data/train_data_char_pad.pkl',
                                                       'sick_data/train_data_flags_pad.pkl')
    ...
    mm = mp.MatchModel(vocab_size=100000,tr_w_embed_dim=300,tr_w_emb_dim_flag=100,char_size=50,char_embed_dim=20,output_dim=100,
                       hidden_dim=200,use_gpu=use_gpu,yt_layer_num=3)
    ...
    print('max dev acc:', max_dev, '\nmax test acc:', max_test)


def test(use_gpu, batch_size, model_path):
    dataset = build_dataset_pytorch.QuoraDataset_train('sick_data/test_data_pad.pkl', 'sick_data/test_data_char_pad.pkl',
                                                       'sick_data/test_data_flags_pad.pkl')
    dataloader = DataLoader(dataset, batch_size, shuffle=True, num_workers=0)
...
def dev(use_gpu, batch_size, model_path):
    dataset = build_dataset_pytorch.QuoraDataset_train('sick_data/dev_data_pad.pkl', 'sick_data/dev_data_char_pad.pkl',
                                                       'sick_data/dev_data_flags_pad.pkl')
    dataloader = DataLoader(dataset, batch_size, shuffle=True, num_workers=0)
...
if __name__ == '__main__':
    train(True, None, 0.0001, 20, 'model/mm-s', 50)
    # test(True, 200, 'model/mm-0_9.pth')
