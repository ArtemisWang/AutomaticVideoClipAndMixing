...
class MatchModel(nn.Module):
    def __init__(self,vocab_size,tr_w_embed_dim,tr_w_emb_dim_flag,char_size, char_embed_dim,output_dim,hidden_dim,use_gpu, yt_layer_num, mode = 'train'):
        ...
        self.fc = nn.Sequential(nn.Linear(30 * 300, 800),
                                ...
                                nn.ReLU(inplace=False),
                                nn.Linear(200, 2))
        # self.fc_0 = nn.Sequential(nn.Linear(30 * 300, 1800),
        #                         nn.ReLU(inplace=False))
        # self.fc_1 = nn.Sequential(nn.Linear(1800+(yt_layer_num+1)*output_dim*2, 800),
        #                         nn.Dropout(p=self.dropout),
        #                         nn.ReLU(inplace=False),
        #                         nn.Linear(800, 200),
        #                         nn.Dropout(p=self.dropout),
        ...
        #                         nn.Linear(200, 2)
        #                         )
        self.conv_p = nn.Sequential(nn.Conv2d(1, 16, 5),
                                    nn.Dropout(p=self.dropout),
                                    nn.ReLU(inplace=False),
                                    nn.MaxPool2d((2, 2)),
                                    nn.Conv2d(16, 4, 4),
                                    ...
                                    nn.ReLU(inplace=False),
                                    nn.MaxPool2d((2, 2)))
        self.conv_nn_p = nn.Sequential(nn.Linear(100, 3000),
                                       nn.Dropout(p=self.dropout),
                                       nn.ReLU(inplace=False))
        self.conv_nn_q = nn.Sequential(nn.Linear(100, 3000),
                                       nn.Dropout(p=self.dropout),
                                       nn.ReLU(inplace=False))
        self.conv_q = nn.Sequential(nn.Conv2d(1, 16, 5),
                                    nn.Dropout(p=self.dropout),
                                    nn.ReLU(inplace=False),
                                    nn.MaxPool2d((2, 2)),
                                    nn.Conv2d(16, 4, 4),
    ...
    def _ytlayer(self, input_p, input_q, i):
        ...
        if emb_dim == self.embedding_dim: ##n51
            ...
            # conv_p = conv_p.view(conv_p.size()[0], -1)
            # conv_q = conv_q.view(conv_q.size()[0], -1)
            # self.conv_out_p = conv_p
            # self.conv_out_q = conv_q
            # conv_p = self.conv_nn_p(conv_p)
            # conv_p = conv_p.view(conv_p.size()[0], 30, -1)
            # conv_q = self.conv_nn_q(conv_q)
            # conv_q = conv_q.view(conv_q.size()[0], 30, -1)
            ...
            # output_p = conv_p
            # output_q = conv_q
            # output_p = t.cat([output_lstm_p, output_linear2_p, output_linear1_p, self.res_p], 2)
            # output_q = t.cat([output_lstm_q, output_linear2_q, output_linear1_q, self.res_q], 2)
        else: ## emb_dim = 800
            ...
            conv_p = conv_p.view(conv_p.size()[0], -1)
            conv_q = conv_q.view(conv_q.size()[0], -1)
            # self.conv_out_p = t.cat([self.conv_out_p, conv_p], 1)
            # self.conv_out_q = t.cat([self.conv_out_q, conv_q], 1)
            # conv_p = self.conv_nn_p(conv_p)
            # conv_p = conv_p.view(conv_p.size()[0], 30, -1)
            # conv_q = self.conv_nn_q(conv_q)
            # conv_q = conv_q.view(conv_q.size()[0], 30, -1)
    ...
    def _fclayer(self, p, q):
        ...
        # print(output.size(), self.conv_out_p.size(), self.conv_out_q.size())
        # output = t.cat([output, self.conv_out_p, self.conv_out_q], 1)
