...
class MatchModel(nn.Module):
    def __init__(self,vocab_size,tr_w_embed_dim,tr_w_emb_dim_flag,char_size, char_embed_dim,output_dim,hidden_dim,use_gpu, yt_layer_num, mode = 'train'):
        ...
        #                            nn.Linear(200, 50),
        #                            nn.ReLU(inplace=False))
        self.fc = nn.Sequential(nn.Linear((yt_layer_num+1)*100, 100),
    ...
    def _ytlayer(self, input_p, input_q, i):
        ...
        if emb_dim == self.embedding_dim: ##n51
            ...
            output_attention_p, output_attention_q = self._attention_block(output_lstm_p, output_lstm_q)
            ...
            # output_p = t.cat([output_lstm_p, conv_p, output_linear2_p, output_linear1_p, self.res_p], 2)
            # output_q = t.cat([output_lstm_q, conv_q, output_linear2_q, output_linear1_q, self.res_q], 2)
            ...
            layer1_lstm_q, layer1_linear1_q, layer1_linear2_q, layer1_conv_q = self.yt_layers_q[i*4],self.yt_layers_q[i*4+1],self.yt_layers_q[i*4+2],self.yt_layers_p[i*4+3]
            ...
            # output_p = t.cat([output_lstm_p, conv_p], 2)
            # output_q = t.cat([output_lstm_q, conv_q], 2)
        return output_attention_p, output_attention_q, conv_p


    def _fclayer(self, conv):
        # p = self.p_fc0(p)
        # q = self.q_fc0(q)
    ...
    def forward(self, output_dim, p_input, q_input):
        ...
        p, q, conv = self._ytlayer(multi_embedding_p, multi_embedding_q, None)
        # print('b:',p.shape, q.shape)
        for i in range(self.layer_num):
            p, q, conv_ = self._ytlayer(p, q, i)
            conv = t.cat([conv, conv_], 1)
        # print('c:',p.shape, q.shape)
        output = self._fclayer(conv)
