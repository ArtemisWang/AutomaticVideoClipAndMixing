...
class MatchModel(nn.Module):
    ...
    def _make_ytlayer(self, input_dim):
        ...
        conv = nn.Sequential(nn.Conv2d(1,2,5),
                             ...
                             nn.ReLU(inplace=False),
                             nn.MaxPool2d((2, 2)))
        return lstm, linear1, linear2, linear0, conv
    ...
    def _ytlayer(self, input_p, input_q, i):
        ...
        if emb_dim == self.embedding_dim: ##n51
            ...
            output_p = t.cat([output_lstm_p, output_linear2_p, output_linear1_p, res_p, conv_p], 2)
            output_q = t.cat([output_lstm_q, output_linear2_q, output_linear1_q, res_q, conv_q], 2)
            # output_p = t.cat([output_lstm_p, conv_p], 2)
            # output_q = t.cat([output_lstm_q, conv_q], 2)
        else: ## emb_dim = 800
            layer1_lstm_p, layer1_linear1_p, layer1_linear2_p, layer1_linear0_p, layer1_conv_p = \
                self.yt_layers_p[i*5],self.yt_layers_p[i*5+1],self.yt_layers_p[i*5+2],self.yt_layers_p[i*5+3],self.yt_layers_p[i*5+4]
            layer1_lstm_q, layer1_linear1_q, layer1_linear2_q, layer1_linear0_q, layer1_conv_q = \
                self.yt_layers_q[i*5],self.yt_layers_q[i*5+1],self.yt_layers_q[i*5+2],self.yt_layers_q[i*5+3],self.yt_layers_q[i*5+4]
