...
class MatchModel(nn.Module):
    def __init__(self,vocab_size,tr_w_embed_dim,tr_w_emb_dim_flag,char_size, char_embed_dim,output_dim,hidden_dim,use_gpu, yt_layer_num, mode = 'train'):
        ...
        self.dropout = 0.5
        self.seq_len = 30
        self.hier_conv_dim = (yt_layer_num + 1) * 400
        ...
        self.q_fc0 = nn.Sequential(nn.Linear(emb_dim_fc, 200),
                                   ...
                                   nn.ReLU(inplace=False),
                                   nn.Linear(200, 50))
        self.fc_rnn = nn.Linear(self.seq_len * 300, self.hier_conv_dim*2)
        self.fc = nn.Sequential(nn.Linear(self.hier_conv_dim*2, 800),
                                nn.Dropout(p=self.dropout),
                                nn.ReLU(inplace=False),
                                nn.Linear(800, 200),
                                nn.Dropout(p=self.dropout),
        ...
        for i in range(yt_layer_num+1):
            ...
            conv = nn.Sequential(nn.Conv2d(1, 32, 5),
                                 nn.Dropout(p=self.dropout),
                                 nn.Conv2d(32, 128, 5),
                                 nn.Dropout(p=self.dropout),
                                 nn.MaxPool2d((2, 2)),
                                 nn.Conv2d(128, 32, 4),
                                 nn.Dropout(p=self.dropout),
                                 nn.Conv2d(32, 16, 4))  ## out_shape:[batch_size, 16, 5, 5]
    ...
    def _ytlayer(self, input_p, input_q, i):
        ...
        if emb_dim == self.embedding_dim: ##n51
            ...
            layer1_lstm_q, layer1_linear1_q, layer1_linear2_q, layer1_linear0_q = self.yt_layers_q[i*4],self.yt_layers_q[i*4+1],self.yt_layers_q[i*4+2],self.yt_layers_q[i*4+3]
    ...
    def _fclayer(self, p, q, conv):
        ...
        out = self.fc_rnn(fc.view([batch_size, seq_len*hidden_dim]))
        # out = t.cat([out, conv], 1)
        out = self.fc(out)
        return out
    ...
    def forward(self, output_dim, p_input, q_input):
        ...
        conv_t = self.conv[0](self._cos_block(p[:,:,:400], q[:,:,:400]))
        batch_size = conv_t.shape[0]
        conv = conv_t.view([batch_size, -1])
        ...
        for i in range(self.layer_num):
            ...
            conv_t = self.conv[i+1](self._cos_block(p[:,:,:400], q[:,:,:400]))
            conv = t.cat([conv_t.view([batch_size, -1]), conv], 1)
        # print('c:',p.shape, q.shape)
        output = self._fclayer(p, q, conv)
        # print(output)
