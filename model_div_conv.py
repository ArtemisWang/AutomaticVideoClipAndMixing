...
class MatchModel(nn.Module):
    def __init__(self,vocab_size,tr_w_embed_dim,tr_w_emb_dim_flag,char_size, char_embed_dim,output_dim,hidden_dim,use_gpu, yt_layer_num, mode = 'train'):
        ...
        self.tr_w_embedding = nn.Embedding(vocab_size, tr_w_embed_dim, max_norm=1.0, norm_type=2, padding_idx=0)  # 输入： LongTensor (N, W), N = mini-batch, W = 每个mini-batch中提取的下标数
        ...
        if self.tr_w_emb_dim_flag != 0:
            self.tr_w_embedding1 = nn.Embedding(vocab_size, tr_w_emb_dim_flag, max_norm=1.0, norm_type=2, padding_idx=0)
        ...
        self.layer0_lstm_p, self.layer0_linear1_p, self.layer0_linear2_p = self._make_layer(self.embedding_dim)
        self.layer0_lstm_q, self.layer0_linear1_q, self.layer0_linear2_q = self._make_layer(self.embedding_dim)
        ...
        self.q_fc0 = nn.Sequential(nn.Linear(emb_dim_fc, 200),
                                   ...
                                   nn.ReLU(inplace=False),
                                   nn.Linear(200, 50))
        self.fc = nn.Sequential(nn.Linear(30 * 300, 800),
                                nn.Dropout(p=self.dropout),
                                nn.ReLU(inplace=False),
                                nn.Linear(800, 200),
                                nn.Dropout(p=self.dropout),
        ...
        #                             nn.MaxPool2d((2, 2)),
        #                             nn.Conv2d(4, 4, 3))
        self.linear_p = nn.Sequential(nn.Linear(output_dim, 30),
                                      nn.Dropout(p=self.dropout),
                                      nn.ReLU(inplace=False))
        self.linear_q = nn.Sequential(nn.Linear(output_dim, 30),
                                      nn.Dropout(p=self.dropout),
                                      nn.ReLU(inplace=False))
    ...
    def _make_layer(self, input_dim):
        lstm = nn.GRU(input_dim, self.hidden_dim, num_layers=2, batch_first=True,dropout=self.dropout, bidirectional=True)
    ...
    def _make_ytlayer(self, input_dim):
        ...
        #                      nn.MaxPool2d((2, 2)),
        #                      nn.Conv2d(4, 4, 3))
        linear3 = nn.Sequential(nn.Linear(self.output_dim, 30),
                                      nn.Dropout(p=self.dropout),
                                      nn.ReLU(inplace=False))
        return lstm, linear1, linear2, conv, linear3
    ...
    def _ytlayer(self, input_p, input_q, i):
        ...
        if emb_dim == self.embedding_dim: ##n51
            ...
            # cosine_matrix = t.unsqueeze(t.bmm(q_input_norm, p_input_norm.transpose(1, 2)), 1)
            conv_p = t.unsqueeze(self.linear_p(output_linear1_p), 1)
            conv_p = self.conv_p(conv_p)
            conv_p = conv_p.view(conv_p.size()[0], 1, -1)
            conv_p = conv_p.repeat(1,30, 1)
            conv_q = t.unsqueeze(self.linear_q(output_linear1_q), 1)
            conv_q = self.conv_q(conv_q)
            ...
            layer1_lstm_p, layer1_linear1_p, layer1_linear2_p, layer1_conv_p, layer1_linear3_p = self.yt_layers_p[i*5],self.yt_layers_p[i*5+1],self.yt_layers_p[i*5+2],self.yt_layers_p[i*5+3],self.yt_layers_p[i*5+4]
            layer1_lstm_q, layer1_linear1_q, layer1_linear2_q, layer1_conv_q, layer1_linear3_q = self.yt_layers_q[i*5],self.yt_layers_q[i*5+1],self.yt_layers_q[i*5+2],self.yt_layers_q[i*5+3],self.yt_layers_p[i*5+4]
            ...
            # conv = self._cos_block(input_p, input_q)
            conv_p = t.unsqueeze(layer1_linear3_p(output_linear1_p), 1)
            conv_p = layer1_conv_p(conv_p)
            conv_p = conv_p.view(conv_p.size()[0], 1, -1)
            conv_p = conv_p.repeat(1, 30, 1)
            conv_q = t.unsqueeze(layer1_linear3_q(output_linear1_q), 1)
            conv_q = layer1_conv_q(conv_q)
            conv_q = conv_q.view(conv_q.size()[0], 1, -1)
            conv_q = conv_q.repeat(1, 30, 1)
            output_p = t.cat([output_lstm_p, output_linear2_p, output_linear1_p, self.res_p, conv_p], 2)
            output_q = t.cat([output_lstm_q, output_linear2_q, output_linear1_q, self.res_q, conv_q], 2)
